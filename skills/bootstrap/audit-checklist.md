...
| GitHub Actions app is in default-branch ruleset bypass | yes | For each ruleset returned by `gh api "repos/<owner>/<repo>/rulesets?includes_parents=true" --jq '.[] \| select(.target=="branch" and (.conditions.ref_name.include \| index("~DEFAULT_BRANCH")))'`, confirm `bypass_actors[]` contains `{ actor_id: 15368, actor_type: "Integration" }` (the `github-actions[bot]` app). When missing, emit a realignment-gap warning entry: release-please targets the release-PR merge commit on the default branch, and any branch ruleset without the Actions app in bypass will cause `POST /releases` to fail with `Resource not accessible by integration`. Remediation: add the GitHub Actions app to the ruleset's bypass list in the UI or via a `PATCH` to `repos/<owner>/<repo>/rulesets/<id>`. |
