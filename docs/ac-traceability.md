...
Test coverage and per-AC verification rows – a `## Test coverage` matrix with `Unit` plus the platform columns the project supports, symbolic status cells, platform test rows (`- <Platform> test – <runner> | <env> | <verifier> | <ISO>`), every known operator-needed manual test as an unchecked imperative operator-review checkbox, every existing manual-review instruction preserved under its AC, and every known test gap as an unchecked imperative operator-review checkbox – are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the coverage matrix, slim-test grammar, per-AC unit-test detail rule, manual-test operator-step rule, manual-review preservation rule, manual-test and test-gap checkbox rule, checkbox imperative-style rule, status-symbol rule, and gap-acknowledgement rule; do not duplicate that grammar here.
