...
Per-AC verification rows — inline per-platform evidence rows and a `- [ ] ⚠️ E2E gap: ...` row that is included only when automated coverage has a real gap (and, when present, placed directly above `- [ ] Manual test: ...`) — are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the slim-evidence grammar and the gap-acknowledgement rule; do not duplicate that grammar here.
