...
Test coverage and per-AC verification rows – a `## Test coverage` matrix with `Unit` plus the platform columns the project supports, symbolic status cells, platform test rows (`- <Platform> test – <runner> | <env> | <verifier> | <ISO>`), every known operator-needed manual test as an unchecked operator-review checkbox, and every known test gap as an unchecked operator-review checkbox – are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the coverage matrix, slim-test grammar, per-AC unit-test detail rule, manual-test operator-step rule, manual-test and test-gap checkbox rule, status-symbol rule, and gap-acknowledgement rule; do not duplicate that grammar here.
