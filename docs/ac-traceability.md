...
Test coverage and per-AC verification rows – a `## Test coverage` matrix with `Unit` plus the affected supported-platform columns, symbol-only status cells, compact platform test rows (`- <Platform> test – <runner>, <environment>[, <verifier or ISO>]`) or explicit gap checkboxes for missing relevant platform validation, every known test gap as an unchecked imperative checkbox for the operator, every known operator action below gaps as an unchecked imperative `Operator check:` checkbox, and every existing manual-review or manual-test instruction preserved under its AC as an operator check – are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the coverage matrix, slim-test grammar, per-AC unit-test detail rule, per-AC content order, operator-check rule, checkbox imperative-style rule, status-symbol rule, matrix consistency rule, platform-evidence-or-gap rule, review-finding gap rule, and gap-acknowledgement rule; do not duplicate that grammar here.
