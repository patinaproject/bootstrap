...
Test coverage and per-AC verification rows – a `## Test coverage` matrix with `Unit` plus the affected supported-platform columns, symbol-only status cells, platform test rows (`- <Platform> test – <runner> | <env> | <verifier> | <ISO>`) or explicit gap checkboxes for missing relevant platform validation, every known test gap as an unchecked imperative operator-review checkbox, every known operator evidence-review step below gaps as an unchecked imperative `Manual review:` checkbox, every known operator-needed product test below reviews as an unchecked imperative `Manual test:` checkbox with an expected result, and every existing manual-review instruction preserved under its AC as the correct checkbox type – are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the coverage matrix, slim-test grammar, per-AC unit-test detail rule, per-AC content order, manual-test operator-step rule, manual-review rule, manual-review preservation rule, manual-test and test-gap checkbox rule, checkbox imperative-style rule, status-symbol rule, matrix consistency rule, platform-evidence-or-gap rule, review-finding gap rule, and gap-acknowledgement rule; do not duplicate that grammar here.
