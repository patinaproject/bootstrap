...
Test coverage and per-AC verification rows – a `## Test coverage` matrix with `Unit` plus only the platform columns the project supports, inline unit/platform test rows (`- <Unit-or-platform> test – <runner> | <env> | <verifier> | <ISO>`), a manual-test row, and an optional `- ⚠️ Test gap: ...` row that is included only when automated coverage has a real gap – are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the coverage matrix, slim-test grammar, and gap-acknowledgement rule; do not duplicate that grammar here.
