...
Test coverage and per-AC verification rows – a `## Test coverage` matrix with `Unit` plus the platform columns the project supports, status cells that use the template's symbols instead of words, optional platform test rows (`- <Platform> test – <runner> | <env> | <verifier> | <ISO>`), manual-test rows only for operator-performed steps, and an optional `- ⚠️ Test gap: ...` row that is included only when automated coverage has a real gap – are defined by the canonical PR template at [`.github/pull_request_template.md`](../.github/pull_request_template.md). The template comments are the source of truth for the coverage matrix, slim-test grammar, per-AC unit-test detail omission rule, manual-test operator-step rule, status-symbol rule, and gap-acknowledgement rule; do not duplicate that grammar here.
